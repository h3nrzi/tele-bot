...
- [x] Bank account service: `getActiveAccount()` returns the single row where `is_active = true`, or `null` if none exists.
- [x] `/setcard` Admin command uses the grammY `conversations` plugin to collect fields in sequence: card number (16-digit validation), card holder name (non-empty), bank name (non-empty), and optional additional notes (user may skip with a designated keyword or empty message). The conversation can be cancelled at any step.
