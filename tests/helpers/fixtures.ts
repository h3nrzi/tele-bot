...
import type { InitiateTopUpInput, InitiateTopUpResult } from "@/modules/top-up/dtos/top-up.dto";
...
function getContainer(containerOrDb: DependencyContainer | DbClient): DependencyContainer {
	...
	return createAppContainer({
		dbClient: containerOrDb as DbClient,
		child: true,
	});
}
...
export async function getTestActiveAccount(containerOrDb: DependencyContainer | DbClient): Promise<BankAccount | null> {
	const container = getContainer(containerOrDb);
	const service = container.resolve(BankAccountService);
...
export async function getTestCurrentRate(containerOrDb: DependencyContainer | DbClient): Promise<ExchangeRate | null> {
	const container = getContainer(containerOrDb);
	const service = container.resolve(ExchangeRateService);
...
export async function listTestCatalogItems(containerOrDb: DependencyContainer | DbClient): Promise<CatalogItem[]> {
	const container = getContainer(containerOrDb);
	const service = container.resolve(CatalogService);
