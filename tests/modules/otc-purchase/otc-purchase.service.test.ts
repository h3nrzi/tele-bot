...
describe("OtcPurchaseService", () => {
	...
	it("happy path: executes OTC purchase, transitions to COMPLETED, records execution details and triggers notifySuccess", async () => {
		...
		const [dbRow] = await db.select().from(wallexOtcPurchases).where(eq(wallexOtcPurchases.id, purchase.id));
	...
	it("wallex failure: transitions to FAILED, records error_message and triggers notifyFailure", async () => {
		...
		(mockWallexClient.placeOtcOrder as any).mockRejectedValue(new Error("Insufficient TMN balance on Wallex"));
		...
		expect(notifyFailure).toHaveBeenCalledWith(purchase, "Insufficient TMN balance on Wallex");
	...
	it("retry: inserts new PENDING row for failed purchase, executes to COMPLETED, and preserves original FAILED row", async () => {
		...
		(mockWallexClient.placeOtcOrder as any).mockRejectedValueOnce(new Error("Connection timeout"));
	...
	it("retry guards: throws error if purchase is not found or not in FAILED state", async () => {
		// 1. Not found
		await expect(service.retry("00000000-0000-0000-0000-000000000000")).rejects.toThrow(OtcPurchaseNotFoundError);
		...
		await expect(service.retry(completedPurchase.id)).rejects.toThrow(InvalidOtcPurchaseStateError);
	});
	...
	it("duplicate prevention: disallows duplicate active purchases for the same top-up request", async () => {
		...
		await expect(service.execute(topUpRequest)).rejects.toThrow(DuplicateActiveOtcPurchaseError);
	});
});
