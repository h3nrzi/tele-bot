...
describe("LedgerService", () => {
	...
	it("records order refund with BUYER_WALLET CREDIT and SYSTEM_CASH DEBIT, and updates original tx reversed_by", async () => {
		...
		expect(mockLedgerRepo.findOriginalByOrderId).toHaveBeenCalledWith("order_refund_1", expect.anything());
		expect(mockLedgerRepo.updateReversedBy).toHaveBeenCalledWith("tx_original_debit", "tx_refund_1", expect.anything());
	});
});
