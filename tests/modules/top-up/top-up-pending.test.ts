...
describe("Admin Pending Top-Up Queue Service", () => {
	...
	async function createBuyerAndSubmit(chatId: bigint, username: string | null, amount: string) {
		...
		return await topUpService.submitReceipt({
			userId: buyer.id,
			fileId: `file_${chatId}`,
	...
	it("filters out non-PENDING requests (INITIATED, APPROVED, REJECTED, CANCELLED, EXPIRED)", async () => {
		...
		const { buyer: initBuyer } = await buyerService.register({
			telegramChatId: 200n,
			telegramUsername: "init_buyer",
		});
		...
		await topUpService.approveTopUp({
			topUpRequestId: toApprove.request.id,
			adminTelegramId: adminId,
		});
		...
		await topUpService.rejectTopUp({
			topUpRequestId: toReject.request.id,
			adminTelegramId: adminId,
			rejectionReason: "Bad receipt",
		});

		// 5. Cancelled item
		const { buyer: cancelBuyer } = await buyerService.register({
			telegramChatId: 500n,
			telegramUsername: "cancel_buyer",
		});
		await topUpService.initiateTopUp({
			userId: cancelBuyer.id,
			usdAmount: "70.00",
		});
