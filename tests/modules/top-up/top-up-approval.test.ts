...
describe("Top-Up Approval Service", () => {
	...
	async function seedPendingRequest(amount = "50.00") {
		const { buyer, wallet } = await buyerService.register({
			...
			telegramUsername: "approval_buyer",
		});
		await exchangeRateService.setRate({
			adminTelegramId: adminId1,
			irrPerUsd: 600000n,
		});
		const { request: initReq } = await topUpService.initiateTopUp({
			userId: buyer.id,
			usdAmount: amount,
		});
	...
	it("approves a PENDING request atomically: transitions status, credits wallet balance, and creates double-entry ledger records in one transaction", async () => {
		...
		const [dbRequest] = await db.select().from(topUpRequests).where(eq(topUpRequests.id, request.id));
		...
		const dbEntries = await db.select().from(ledgerEntries).where(eq(ledgerEntries.ledgerTransactionId, tx.id));
	...
	it("accumulates wallet balance correctly on multiple top-ups", async () => {
		...
		const { request: init2 } = await topUpService.initiateTopUp({
			userId: buyer.id,
			usdAmount: "35.50",
		});
		const { request: req2 } = await topUpService.submitReceipt({
			userId: buyer.id,
			fileId: "receipt_2",
		});

		const result2 = await topUpService.approveTopUp({
	...
	it("throws TopUpRequestNotPendingError when request is in INITIATED, APPROVED, REJECTED, CANCELLED, or EXPIRED state", async () => {
		...
		const { request: initiatedReq } = await topUpService.initiateTopUp({
			userId: buyer.id,
			usdAmount: "20.00",
		});
