...
describe("Top-Up Cancellation Service", () => {
	...
	it("throws CannotCancelPendingTopUpError when request is in PENDING status (receipt submitted)", async () => {
		...
		await topUpService.submitReceipt({
			userId: buyer.id,
			fileId: "receipt_123",
		});

		await expect(topUpService.cancelTopUp({ userId: buyer.id })).rejects.toThrow(CannotCancelPendingTopUpError);
	});
	...
	it("throws NoActiveTopUpRequestError when buyer has no active request to cancel", async () => {
		const { buyer } = await buyerService.register({
			telegramChatId: 111333n,
			telegramUsername: "no_req_buyer",
		});

		await expect(topUpService.cancelTopUp({ userId: buyer.id })).rejects.toThrow(NoActiveTopUpRequestError);
	});
});
