...
import { ActiveTopUpRequestExistsError, InvalidTopUpAmountError } from "@/modules/top-up/top-up.errors";
...
describe("Top-Up Initiation Service", () => {
	...
	it("initiates a top-up request with valid amount and calculates IRR amount accurately", async () => {
		...
		// Verify in database
		const [row] = await db.select().from(topUpRequests).where(eq(topUpRequests.id, result.request.id));
	...
	it("rejects initiation if Buyer already has an active INITIATED request", async () => {
		...
		await expect(topUpService.initiateTopUp({ userId: user.id, usdAmount: "100.00" })).rejects.toThrow(
	...
	it("throws NoExchangeRateError when attempting to initiate without active exchange rate", async () => {
		...
		await expect(topUpService.initiateTopUp({ userId: user!.id, usdAmount: "50.00" })).rejects.toThrow(
			NoExchangeRateError,
		);
	});
	...
	it("throws InvalidTopUpAmountError when amount is below configured minimum", async () => {
		...
		await expect(topUpService.initiateTopUp({ userId: user.id, usdAmount: "5.00" })).rejects.toThrow(
	...
	it("throws InvalidTopUpAmountError when amount is above configured maximum", async () => {
		...
		await expect(topUpService.initiateTopUp({ userId: user.id, usdAmount: "1500.00" })).rejects.toThrow(
			InvalidTopUpAmountError,
		);
	});
	...
	it("throws InvalidTopUpAmountError on non-numeric or negative amount", async () => {
		...
		await expect(topUpService.initiateTopUp({ userId: user.id, usdAmount: "-10.00" })).rejects.toThrow(
			InvalidTopUpAmountError,
		);

		await expect(topUpService.initiateTopUp({ userId: user.id, usdAmount: "invalid" })).rejects.toThrow(
			InvalidTopUpAmountError,
		);
	});
	...
	describe("AUTO_SYNC Rate Mode", () => {
		...
		it("throws NoExchangeRateError when OTC quote fails and no baseline rate exists", async () => {
			...
			(mockWallexClient.getOtcPrice as any).mockRejectedValue(new WallexNetworkError("Wallex down"));
