...
describe("Receipt Submission Service", () => {
	...
	it("submits receipt photo and updates status to PENDING atomically", async () => {
		...
		// Verify in DB
		const [row] = await db.select().from(topUpRequests).where(eq(topUpRequests.id, request.id));
	...
	it("throws NoInitiatedTopUpRequestError when request is already in PENDING state (disallows duplicate submission)", async () => {
		...
		await expect(topUpService.submitReceipt({ userId: user.id, fileId: "photo_2" })).rejects.toThrow(
	...
	it("throws TopUpRequestExpiredError and marks request as EXPIRED if expires_at has passed", async () => {
		...
		await db.update(topUpRequests).set({ expiresAt: past }).where(eq(topUpRequests.id, request.id));

		await expect(topUpService.submitReceipt({ userId: user.id, fileId: "photo_expired" })).rejects.toThrow(
