...
describe("ManualRateLock Adapter", () => {
	...
	beforeEach(() => {
		...
		manualRateLock = new ManualRateLock(mockExchangeRateRepo as unknown as IExchangeRateRepository);
	});
	...
	it("no rate exists: throws NoExchangeRateError when findLatest returns null", async () => {
		...
		await expect(manualRateLock.resolve(new Decimal(50))).rejects.toThrow(NoExchangeRateError);
		expect(mockExchangeRateRepo.findLatest).toHaveBeenCalledTimes(1);
	});
