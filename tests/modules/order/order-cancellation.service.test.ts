...
import { OrderNotFoundError, InvalidOrderStatusError, OrderNotOwnedByBuyerError } from "@/modules/order/order.errors";
...
describe("Order Cancellation Service (Ticket 08)", () => {
	...
	it("completes cancellation successfully even if notification callbacks throw errors (resilience)", async () => {
		...
		vi.spyOn(notifier, "onOrderCancelled").mockRejectedValueOnce(new Error("Telegram network error"));
