...
import { InvalidOrderStatusError, OrderAlreadyClaimedError, OrderNotFoundError } from "@/modules/order/order.errors";
import { orderAdminNotifications, orders } from "@/modules/order/order.schema";
import { OrderService } from "@/modules/order/order.service";
import { wallets } from "@/modules/wallet/wallet.schema";
import { claimTestOrder, createTestBuyer, createTestCatalogItem, placeTestOrder } from "@tests/helpers/fixtures";
...
describe("Order Claim Service (Ticket 05)", () => {
	...
	it("rejects claim on orders in terminal status (FULFILLED, REJECTED, CANCELLED)", async () => {
		...
		await db.update(orders).set({ status: "FULFILLED", fulfilledAt: new Date() }).where(eq(orders.id, order1.id));
	...
	it("completes claim successfully even if onOrderClaimed throws", async () => {
		...
		vi.spyOn(notifier, "onOrderClaimed").mockRejectedValueOnce(new Error("Telegram API connection timeout"));
