...
describe("Order Rejection Service (Ticket 07)", () => {
	...
	it("happy path from PLACED: refund ledger written, balance restored, status -> REJECTED, reversed_by link set", async () => {
		...
		await db.update(wallets).set({ availableBalance: initialBalance }).where(eq(wallets.id, wallet.id));
		...
		const { order: placedOrder, ledgerTransaction: debitLedgerTx } = await placeTestOrder(container, {
			userId: buyer.id,
		...
		expect(originalDbTx?.reversedByLedgerTransactionId).toBe(result.refundLedgerTransaction.id);
		...
		const buyerWalletEntry = refundEntries.find((e) => e.accountType === "BUYER_WALLET");
		const systemCashEntry = refundEntries.find((e) => e.accountType === "SYSTEM_CASH");
	...
	it("happy path from PROCESSING: claiming admin rejects order, balance restored, refund ledger written", async () => {
		...
		const { order: placedOrder, ledgerTransaction: originalDebitTx } = await placeTestOrder(container, {
			userId: buyer.id,
			catalogItemId: item.id,
		});
	...
	it("validates OTHER rejection category: requires non-empty rejectionNote", async () => {
		...
		expect(successResult.order.rejectionNote).toBe("Custom reason explanation provided by admin.");
	});
	...
	it("rejects rejection attempt on already-terminal orders (FULFILLED, REJECTED, CANCELLED) with InvalidOrderStatusError", async () => {
		...
		await db.update(orders).set({ status: "CANCELLED", cancelledAt: new Date() }).where(eq(orders.id, order3.id));
	...
	it("completes rejection successfully even if notification callbacks throw errors (resilience)", async () => {
		...
		vi.spyOn(notifier, "onOrderRejected").mockRejectedValueOnce(new Error("Telegram network error"));
