...
describe("Admin Order Queue Service (Ticket 09)", () => {
	...
	it("returns active orders (PLACED and PROCESSING) and excludes terminal orders (FULFILLED, REJECTED, CANCELLED)", async () => {
		...
		// Fund wallets
		await db.update(wallets).set({ availableBalance: "200.00" }).where(eq(wallets.id, wallet1.id));
		await db.update(wallets).set({ availableBalance: "200.00" }).where(eq(wallets.id, wallet2.id));
		await db.update(wallets).set({ availableBalance: "200.00" }).where(eq(wallets.id, wallet3.id));
