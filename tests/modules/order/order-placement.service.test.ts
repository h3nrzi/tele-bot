...
import { CatalogItemUnavailableError, InsufficientBalanceForOrderError } from "@/modules/order/order.errors";
...
describe("Order Placement Service (Ticket 04)", () => {
	...
	it("places an order atomically: debits wallet, creates PLACED order, and writes double-entry ledger entries", async () => {
		...
		await db.update(wallets).set({ availableBalance: "50.00" }).where(eq(wallets.id, initialWallet.id));
		...
		const [dbOrder] = await db.select().from(orders).where(eq(orders.id, result.order.id));
		...
		const [dbWallet] = await db.select().from(wallets).where(eq(wallets.id, initialWallet.id));

		expect(dbWallet?.availableBalance).toBe("35.01");

		// 4. Assert ledger transaction & entries in DB
		const [dbTx] = await db.select().from(ledgerTransactions).where(eq(ledgerTransactions.orderId, result.order.id));
	...
	it("rejects placement when available balance is insufficient at transaction time", async () => {
		...
		await db.update(wallets).set({ availableBalance: "4.00" }).where(eq(wallets.id, wallet.id));
		...
		const [txCount] = await db.select({ value: count() }).from(ledgerTransactions);
	...
	it("resolves buyer via telegramChatId input", async () => {
		...
		await db.update(wallets).set({ availableBalance: "20.00" }).where(eq(wallets.id, wallet.id));
	...
	it("prevents negative balance during concurrent order placements on the same wallet", async () => {
		...
		const rejectedResult = [result1, result2].find((r) => r.status === "rejected") as PromiseRejectedResult;
		expect(rejectedResult.reason).toBeInstanceOf(InsufficientBalanceForOrderError);
	...
	it("completes order placement successfully even if admin notification dispatch fails", async () => {
		...
		vi.spyOn(notifier, "onOrderPlaced").mockRejectedValueOnce(new Error("Telegram network outage"));
