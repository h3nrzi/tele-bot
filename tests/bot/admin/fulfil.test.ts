...
describe("Admin Order Fulfilment Handler & Conversation (Ticket 06)", () => {
	...
	function makeMessageUpdate(updateId: number, chatId: number, text: string, username = "admin_user") {
	...
	it("rejects non-claiming Admin tapping [📦 Fulfil Order] with alert and blocks conversation", async () => {
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId2, `order:fulfil:${placedOrder.id}`, 802, "other_admin"),
		);
	...
	it("rejects tapping [📦 Fulfil Order] on order not in PROCESSING state", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(1, adminChatId1, `order:fulfil:${placedOrder.id}`, 801, "admin1"));
	...
	it("happy path: 3-step conversation (input -> preview & confirm -> commit) fulfils order, notifies buyer, and updates admin notifications", async () => {
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId1, `order:fulfil:${placedOrder.id}`, 801, "lead_admin"),
		);
		...
		await bot.handleUpdate(makeMessageUpdate(2, adminChatId1, deliveryContent, "lead_admin"));
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(3, adminChatId1, "fulfil:confirm", 801, "lead_admin"));
	...
	it("re-enter loop: tapping [✗ Re-enter] loops back to Step 1 without submitting until confirmed", async () => {
		...
		// Input draft 1
		await bot.handleUpdate(makeMessageUpdate(2, adminChatId1, "Draft content 1", "lead_admin"));

		expect(repliedMessages[1]).toContain("Draft content 1");

		// Tap [✗ Re-enter]
		await bot.handleUpdate(makeCallbackQueryUpdate(3, adminChatId1, "fulfil:reenter", 801, "lead_admin"));

		expect(repliedMessages[2]).toContain("تحویل سفارش");

		// Input final content
		const finalContent = "Final Correct Credentials Key: 1234-5678";
		await bot.handleUpdate(makeMessageUpdate(4, adminChatId1, finalContent, "lead_admin"));

		expect(repliedMessages[3]).toContain(finalContent);

		// Confirm
		await bot.handleUpdate(makeCallbackQueryUpdate(5, adminChatId1, "fulfil:confirm", 801, "lead_admin"));
	...
	it("cancelling conversation (/cancel) aborts cleanly without updating DB status", async () => {
		...
		// Send /cancel
		await bot.handleUpdate(makeMessageUpdate(2, adminChatId1, "/cancel", "lead_admin"));
