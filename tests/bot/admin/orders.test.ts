...
import { createMockFetch, type MockEditedMessage, type MockAnsweredCallbackQuery } from "@tests/helpers/mock-context";
...
import { getAdminOrderQueueItemKeyboard } from "@/bot/admin/keyboards/order.keyboards";
...
describe("/orders Admin Queue Command (Ticket 09)", () => {
	...
	function makeCommandUpdate(updateId: number, chatId: number, text: string, username = "admin_user") {
	...
	it("action button [▶ Start Processing] from /orders works identically to notification button", async () => {
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId1, `order:process:${placedOrder.id}`, 100, "admin_one"),
		);
	...
	it("action button [📦 Fulfil Order] from /orders opens fulfilment conversation for claiming Admin", async () => {
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId1, `order:fulfil:${placedOrder.id}`, 101, "admin_one"),
		);
	...
	it("action button [✗ Reject] from /orders opens rejection conversation", async () => {
		...
		// Admin 1 taps [✗ Reject]
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId1, `order:reject:${placedOrder.id}`, 102, "admin_one"),
		);
