...
describe("Admin Approval Callback Handler", () => {
	...
	it('multi-admin race: second admin tap shows "already processed" and does not credit wallet again', async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(5, adminChatId2, `approve:${pendingReq!.id}`, 11));
	...
	it("silently ignores callback queries from non-Admins", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(4, nonAdminChatId, `approve:${pendingReq!.id}`, 10));
	...
	it("triggers post-approval OTC purchase execution when admin approves request", async () => {
		...
		const executeSpy = vi.spyOn(OtcPurchaseService.prototype, "execute").mockResolvedValue({} as any);
