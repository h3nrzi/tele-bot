...
describe("Admin Rejection Conversation & Callback Handler", () => {
	...
	function createTestBot(customFetch?: typeof fetch) {
		...
		const { fetch: mockFetch } = createMockFetch(repliedMessages, sentPhotos, editedMessages, answeredCallbackQueries);
	...
	it("happy path: admin clicks Reject -> chooses preset reason -> request is REJECTED, buyer is notified, original admin message edited, wallet unchanged, 0 ledger rows", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(5, adminChatId1, "reject_reason:wrong_amount", 20));

		// 3. Verify DB state: REJECTED, rejectionReason = "مبلغ واریزی اشتباه است", wallet = 0.00
		const [rejectedReq] = await db.select().from(topUpRequests).where(eq(topUpRequests.id, pendingReq.id));
		...
		const [buyerWallet] = await db.select().from(wallets).where(eq(wallets.userId, pendingReq.userId));
		...
		expect(repliedMessages.some((msg) => msg.includes("رد شد") && msg.includes("مبلغ واریزی اشتباه است"))).toBe(true);
	...
	it("custom note path: admin chooses Other / custom... -> enters free text note -> request is REJECTED with custom note verbatim", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(5, adminChatId1, "reject_reason:custom", 20));
		...
		await bot.handleUpdate(makeTextUpdate(6, adminChatId1, customNote, "super_admin"));
		...
		expect(repliedMessages.some((msg) => msg.includes("رد شد") && msg.includes(customNote))).toBe(true);
	...
	it("cancellation via Cancel button leaves request in PENDING status without notifying buyer", async () => {
		...
		// 1. Admin taps Reject
		await bot.handleUpdate(makeCallbackQueryUpdate(4, adminChatId1, `reject:${pendingReq.id}`, 10));

		// 2. Admin taps Cancel button
		await bot.handleUpdate(makeCallbackQueryUpdate(5, adminChatId1, "reject_reason:cancel", 20));
		...
		const [stillPendingReq] = await db.select().from(topUpRequests).where(eq(topUpRequests.id, pendingReq.id));
	...
	it("silently ignores reject callback queries from non-Admins", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(4, nonAdminChatId, `reject:${pendingReq.id}`, 10));
	...
	it("multi-Admin race: if request is already approved or rejected, rejecting admin sees already processed outcome", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(4, adminChatId1, `approve:${pendingReq.id}`, 10));

		// Second Admin tries to reject
		await bot.handleUpdate(makeCallbackQueryUpdate(5, adminChatId2, `reject:${pendingReq.id}`, 11));

		// Second admin selects preset reason
		await bot.handleUpdate(makeCallbackQueryUpdate(6, adminChatId2, "reject_reason:wrong_amount", 21));

		// Request is still APPROVED
		const [approvedReq] = await db.select().from(topUpRequests).where(eq(topUpRequests.id, pendingReq.id));
	...
	it("buyer push notification failure does not roll back rejection transaction", async () => {
		...
		const failingFetch: typeof fetch = async (url: any, init?: any) => {
			...
			if (method === "sendMessage") {
				...
				return new Response(
					JSON.stringify({
						ok: true,
						result: { message_id: 1, text: body.text },
					}),
					{
						status: 200,
		...
		// Admin rejects
		await bot.handleUpdate(makeCallbackQueryUpdate(4, adminChatId1, `reject:${pendingReq.id}`, 10));
		await bot.handleUpdate(makeCallbackQueryUpdate(5, adminChatId1, "reject_reason:unreadable_receipt", 20));
