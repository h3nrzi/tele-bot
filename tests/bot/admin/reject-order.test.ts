...
describe("Admin Order Rejection Handler & Conversation (Ticket 07)", () => {
	...
	it("rejects tapping [✗ Reject] on terminal orders with an alert", async () => {
		...
		await db.update(orders).set({ status: "FULFILLED", fulfilledAt: new Date() }).where(eq(orders.id, placedOrder.id));

		const { bot, answeredCallbackQueries, repliedMessages } = createTestBot();

		await bot.handleUpdate(makeCallbackQueryUpdate(1, adminChatId1, `order:reject:${placedOrder.id}`, 801, "admin1"));
	...
	it("happy path from PLACED: admin selects preset category + skips note -> order REJECTED, refund written, buyer notified, admin notifications edited", async () => {
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId1, `order:reject:${placedOrder.id}`, 801, "lead_admin"),
		);
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(2, adminChatId1, "order_reject_cat:OUT_OF_STOCK", 801, "lead_admin"),
		);
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(3, adminChatId1, "order_reject_note:skip", 801, "lead_admin"));
	...
	it("happy path from PROCESSING: any admin rejects with OTHER category + text note", async () => {
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId2, `order:reject:${placedOrder.id}`, 802, "second_admin"),
		);

		expect(repliedMessages[0]).toContain("رد سفارش");

		// Admin 2 selects OTHER category
		await bot.handleUpdate(makeCallbackQueryUpdate(2, adminChatId2, "order_reject_cat:OTHER", 802, "second_admin"));
		...
		await bot.handleUpdate(makeMessageUpdate(3, adminChatId2, customReason, "second_admin"));
	...
	it("cancelling conversation (/cancel) aborts cleanly without altering order status or balance", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(2, adminChatId1, "flow:cancel", 801, "admin1"));
