...
describe("/pending Admin Queue and Review Callback", () => {
	...
	it("Review button callback: re-sends receipt photo with Approve/Reject buttons, allowing admin approval directly", async () => {
		...
		const { bot, sentPhotos, repliedMessages, editedMessages, answeredCallbackQueries } = createTestBot();
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(4, adminChatId, `review:${pendingReq!.id}`, 15));
	...
	it("silently ignores /pending, review, and pending_page callbacks from non-Admins", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(5, nonAdminChatId, `review:${pendingReq!.id}`));

		// 4. Non-admin sends pagination callback
		await bot.handleUpdate(makeCallbackQueryUpdate(6, nonAdminChatId, "pending_page:2"));

		// Should not reply to /pending from non-admin
		const nonAdminReplies = repliedMessages.filter((m) => m.includes("صف درخواست‌های در انتظار"));
	...
	it("Review callback on non-existent requestId answers with alert", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(1, adminChatId, "review:00000000-0000-0000-0000-000000000000"));
	...
	it("Pagination callback when queue has become empty edits message to empty queue message", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(1, adminChatId, "pending_page:2"));
