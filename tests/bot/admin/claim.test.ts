...
describe("Admin Order Claim & Start Processing Callback Handler (Ticket 05)", () => {
	...
	it("happy path: admin taps [▶ Start Processing] -> transitions order to PROCESSING and edits all admin notifications", async () => {
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId1, `order:process:${placedOrder.id}`, 801, "lead_admin"),
		);
	...
	it('multi-admin race: second admin tap shows "already claimed" alert', async () => {
		...
		// 1. Admin 1 claims
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId1, `order:process:${placedOrder.id}`, 801, "fast_admin"),
		);

		// 2. Admin 2 taps on their copy of the notification
		await bot.handleUpdate(
			makeCallbackQueryUpdate(2, adminChatId2, `order:process:${placedOrder.id}`, 802, "slow_admin"),
		);
	...
	it("silently ignores callback queries from non-Admins", async () => {
		...
		// Non-admin taps
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, nonAdminChatId, `order:process:${placedOrder.id}`, 999, "imposter"),
		);
	...
	it("shows alert when attempting to claim terminal order", async () => {
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId1, `order:process:${placedOrder.id}`, 801, "admin_user"),
		);
	...
	it("answers non-interactive order:noop button cleanly", async () => {
		...
		await bot.handleUpdate(makeCallbackQueryUpdate(1, adminChatId1, "order:noop", 801, "admin_user"));
	...
	it("answers order:fulfil callback when order is not found", async () => {
		...
		await bot.handleUpdate(
			makeCallbackQueryUpdate(1, adminChatId1, "order:fulfil:00000000-0000-0000-0000-000000000000", 801, "admin_user"),
		);
