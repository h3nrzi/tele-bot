...
import { handleCancelCommand } from "@/bot/buyer";
...
describe("/cancel Command Handler", () => {
	...
	it("cancels INITIATED top-up request and replies with cancellation confirmation message", async () => {
		...
		const initResult = await initiateTestTopUp(container, {
			userId: buyer.id,
			usdAmount: "50.00",
		});
		...
		const [row] = await db.select().from(topUpRequests).where(eq(topUpRequests.id, initResult.request.id));
	...
	it("informs buyer when there is no active top-up request to cancel", async () => {
		...
		await createTestBuyer(container, {
			telegramChatId: chatId,
			telegramUsername: "charlie_buyer",
		});
