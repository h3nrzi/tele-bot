...
describe("Receipt Submission & Admin Push Notification Handler", () => {
	...
	function createTestBot(mockFetchOverride?: typeof fetch) {
		...
		const mockFetch = mockFetchOverride || createMockFetch(repliedMessages, sentPhotos).fetch;
	...
	it("happy path: buyer submits receipt photo with caption -> PENDING transition + admin notifications with Approve/Reject inline keyboard", async () => {
		...
		const approveBtn = flatButtons.find((btn: any) => btn.callback_data === `approve:${requestRow?.id}`);
		const rejectBtn = flatButtons.find((btn: any) => btn.callback_data === `reject:${requestRow?.id}`);
	...
	it("expired request: buyer uploads photo after expiry -> EXPIRED transition + expiry error message to Buyer + no admin notification", async () => {
		...
		await db.update(topUpRequests).set({ expiresAt: past }).where(eq(topUpRequests.id, requestRow!.id));
		...
		const [updatedRow] = await db.select().from(topUpRequests).where(eq(topUpRequests.id, requestRow!.id));
