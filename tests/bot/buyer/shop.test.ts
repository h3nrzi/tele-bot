...
import { createTestBuyer, createTestCatalogItem, placeTestOrder } from "@tests/helpers/fixtures";
...
describe("Buyer /shop Command & Order Confirmation Prompt (Ticket 03)", () => {
	...
	describe("/shop Command via bot.handleUpdate", () => {
		it("shows graceful empty-state message when no active items exist in catalog", async () => {
			...
			await bot.handleUpdate(makeMessageUpdate(1, buyerChatId, "/shop", "Buyer"));
		...
		it("responds to menu button '🛍️ فروشگاه خدمات'", async () => {
			...
			await bot.handleUpdate(makeMessageUpdate(1, buyerChatId, "🛍️ فروشگاه خدمات", "Buyer"));
	...
	describe("Item Selection & Confirmation Prompt (shop:item:<id>)", () => {
		it("shows confirmation prompt with name, description, price, balance, and Confirm button when balance >= price", async () => {
			...
			await db.update(wallets).set({ availableBalance: "25.00" }).where(eq(wallets.userId, buyer.id));
		...
		it("shows error and omits Confirm button when balance < price", async () => {
			...
			await db.update(wallets).set({ availableBalance: "3.00" }).where(eq(wallets.userId, buyer.id));
	...
	describe("Prompt Dismissal / Cancellation (shop:cancel)", () => {
		it("dismisses prompt without any side effects or created orders", async () => {
			...
			await bot.handleUpdate(makeCallbackQueryUpdate(1, buyerChatId, "shop:cancel"));
	...
	describe("Order Confirmation Flow & Placement Handler (shop:confirm:<id>) (Ticket 04)", () => {
		it("places order, debits wallet, writes ledger, replies to buyer, and dispatches push notification to admins with action buttons", async () => {
			...
			await db.update(wallets).set({ availableBalance: "40.00" }).where(eq(wallets.id, wallet.id));
			...
			const { bot, editedMessages, answeredCallbackQueries, sentMessages } = createTestBot();

			await bot.handleUpdate(
				makeCallbackQueryUpdate(1, buyerChatId, `shop:confirm:${item.id}`, 1, "Buyer", "buyer_user"),
			);
			...
			const processButton = flatAdminButtons.find((btn: any) => btn.callback_data === `order:process:${dbOrder!.id}`);
			const rejectButton = flatAdminButtons.find((btn: any) => btn.callback_data === `order:reject:${dbOrder!.id}`);
		...
		it("surfaces error alert when buyer has insufficient balance at confirm time (race condition)", async () => {
			...
			await db.update(wallets).set({ availableBalance: "5.00" }).where(eq(wallets.id, wallet.id));
		...
		it("surfaces error alert when selected item was deactivated prior to confirmation", async () => {
			...
			await bot.handleUpdate(makeCallbackQueryUpdate(1, buyerChatId, `shop:confirm:${item.id}`));
	...
	describe("Admin Order Actions (Ticket 05 Process & Ticket 07 Reject Placeholder)", () => {
		it("handles order:process callback query on placed order", async () => {
			...
			await db.update(wallets).set({ availableBalance: "50.00" }).where(eq(wallets.userId, buyer.id));
