...
import { formatPersianDateTime, formatPersianDate, formatPersianTime, formatTimeAgo } from "@/core/shared/date.utils";
...
describe("Date Utilities", () => {
	describe("formatPersianDateTime", () => {
		...
		it("formats an ISO date string directly", () => {
			expect(formatPersianDateTime("2026-09-02T08:45:05.265Z")).toBe("1405/06/11 12:15:05");
		});
		...
		it("supports includeSeconds: false", () => {
			const date = new Date("2026-09-02T08:45:05.265Z");
			expect(formatPersianDateTime(date, { includeSeconds: false })).toBe("1405/06/11 12:15");
		});

		it("supports custom separator", () => {
			const date = new Date("2026-09-02T08:45:05.265Z");
			expect(formatPersianDateTime(date, { separator: " - " })).toBe("1405/06/11 - 12:15:05");
		});

		it("supports Persian digits when usePersianDigits is true", () => {
			const date = new Date("2026-09-02T08:45:05.265Z");
			expect(formatPersianDateTime(date, { usePersianDigits: true })).toBe("۱۴۰۵/۰۶/۱۱ ۱۲:۱۵:۰۵");
		});
	...
	describe("formatPersianDate", () => {
		...
		it("supports Persian digits for date only", () => {
			const date = new Date("2026-09-02T08:45:05.265Z");
			expect(formatPersianDate(date, { usePersianDigits: true })).toBe("۱۴۰۵/۰۶/۱۱");
		});
