 ...
 */
export async function handleShopCommand(ctx: Context, catalogService: CatalogService): Promise<void> {
 ...
 */
export async function handleShopItemCallback(ctx: Context, deps: ShopItemDependencies): Promise<void> {
	const sender = ctx.from;
	if (!sender) {
		return;
	}
 ...
 */
export async function handleShopConfirmCallback(ctx: Context, deps: ShopConfirmDependencies): Promise<void> {
	const sender = ctx.from;
	if (!sender) {
		return;
	}
	...
	try {
		...
		if (err instanceof CatalogItemUnavailableError) {
			try {
				await ctx.editMessageText("⚠️ این خدمت دیگر در دسترس نیست یا غیرفعال شده است.", {
					reply_markup: new InlineKeyboard().text("✗ انصراف", "shop:cancel"),
				});
			} catch {}
