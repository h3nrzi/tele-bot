 ...
 */
export async function handlePhotoMessage(ctx: Context, deps: PhotoHandlerDependencies): Promise<void> {
	...
	if (activeRequest.status === "PENDING") {
		await ctx.reply("شما قبلاً رسید پرداخت خود را ارسال کرده‌اید و درخواست شما در انتظار بررسی ادمین است.");
		return;
	}
	...
	try {
		...
		const buyerDisplay = sender.username ? `@${sender.username} (شناسه: ${sender.id})` : `شناسه: ${sender.id}`;
		...
		for (const adminId of resolvedAdminIds) {
			try {
				...
				console.error(`Failed to send receipt photo notification to admin ${adminId}:`, err);
			}
		}
	} catch (err: any) {
