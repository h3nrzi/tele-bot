 ...
 */
export async function handleCancelCommand(ctx: Context, deps: CancelHandlerDependencies): Promise<void> {
	...
	try {
		...
		if (err instanceof NoActiveTopUpRequestError) {
			await ctx.reply("شما در حال حاضر هیچ درخواست افزایش موجودی فعالی برای لغو ندارید.", {
				reply_markup: getBuyerWalletMenuKeyboard(),
			});
			return;
		}
