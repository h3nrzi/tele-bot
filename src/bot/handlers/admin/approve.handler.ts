...
async function editAdminMessageOutcome(ctx: Context, outcomeText: string): Promise<void> {
 ...
 */
export async function handleApproveCallback(ctx: Context, deps: ApproveHandlerDependencies): Promise<void> {
		...
		await topUpService.approveTopUp(
			...
			},
			{
				notifyBuyer: async (params) => {
					...
					await ctx.api.sendMessage(params.buyerTelegramChatId.toString(), messageText);
				},
				executeOtcPurchase: deps.otcPurchaseService
					? (request) => {
							void deps.otcPurchaseService!.execute(request).catch((err) => {
								console.error(`Failed to execute post-approval OTC purchase for request ${request.id}:`, err);
							});
						}
					: undefined,
			},
