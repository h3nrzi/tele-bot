...
export function formatAdminOrderPlacedMessage(context: {
  ...
  buyer: { telegramUsername?: string | null; telegramChatId: bigint | number | string };
...
export function formatBuyerOrderRejectedMessage(params: {
  ...
  const categoryInfo =
    params.rejectionCategory in ORDER_REJECTION_CATEGORIES
      ? ORDER_REJECTION_CATEGORIES[
      params.rejectionCategory as OrderRejectionCategoryCode
      ]
      : null;

  const categoryLabel = categoryInfo
    ? `${categoryInfo.label} (${categoryInfo.labelEn})`
    : params.rejectionCategory;

  const noteLine = params.rejectionNote
    ? `💬 توضیحات: ${escapeMarkdown(params.rejectionNote)}\n`
    : '';
...
export class TelegramOrderNotifier implements IOrderNotifier {
  ...
  constructor(
    ...
    if ('api' in optionsOrBotOrApi && (optionsOrBotOrApi as any).api) {
      ...
      optionsOrBotOrApi &&
      typeof (optionsOrBotOrApi as TelegramApiLike).sendMessage === 'function'
    ) {
  ...
  public async onOrderPlaced(context: OnOrderPlacedContext): Promise<void> {
    ...
    for (const adminId of resolvedAdmins) {
      try {
        ...
        )) as { chat: { id: number | bigint | string }; message_id: number | bigint | string };
    ...
    if (this.orderRepo && notificationPayloads.length > 0) {
      try {
        ...
        console.error(
          `Failed to persist admin notifications for order ${context.order.id}:`,
          repoErr
        );
      }
    }
  ...
  public async onOrderFulfilled(context: OnOrderFulfilledContext): Promise<void> {
    ...
    const adminDisplay =
      context.adminUsername || String(context.adminTelegramId);
  ...
  public async onOrderRejected(context: OnOrderRejectedContext): Promise<void> {
    ...
    const adminDisplay =
      ...
      (context.adminTelegramId ? String(context.adminTelegramId) : undefined);
  ...
  private async sendMarkdownWithFallback(
    ...
    try {
      ...
      if (sendErr?.message?.includes("can't parse entities")) {
        ...
        console.error(
          `Failed to send notification to ${recipientLabel} for order ${orderId}:`,
          sendErr
        );
      }
    }
