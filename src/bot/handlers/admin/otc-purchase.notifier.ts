...
export function formatOtcPurchaseSuccessMessage(purchase: OtcPurchase): string {
  ...
  const tmnSpent =
    purchase.wallexExecutedSum !== null
      ? formatIrr(purchase.wallexExecutedSum / 10n)
      : '-';
  const executedPriceTmn =
    purchase.wallexExecutedPrice !== null
      ? formatIrr(purchase.wallexExecutedPrice / 10n)
      : '-';
  const feeTmn =
    purchase.wallexFee !== null
      ? formatIrr(purchase.wallexFee / 10n)
      : '-';
