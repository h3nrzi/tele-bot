 ...
 */
export async function handleRateModeCommand(ctx: Context, deps: RateModeHandlerDependencies): Promise<void> {
	if (!ctx.from) {
		return;
	}
 ...
 */
export async function handleRateModeSwitchCallback(ctx: Context, deps: RateModeHandlerDependencies): Promise<void> {
	...
	if (targetMode === "AUTO_SYNC") {
		...
		try {
			...
			if (ctx.callbackQuery) {
				try {
					await ctx.answerCallbackQuery({
						text: "حالت نرخ ارز با موفقیت به خودکار تغییر یافت.",
					});
				} catch {}
			}
		...
		if (ctx.callbackQuery) {
			try {
				await ctx.answerCallbackQuery({
					text: "حالت نرخ ارز با موفقیت به دستی تغییر یافت.",
				});
			} catch {}
		}
