...
export function isSkipCommand(text: string): boolean {
	...
	return trimmed === "-" || trimmed === "skip" || trimmed.startsWith("/skip") || trimmed === "";
}
...
export function createSetCardConversation(bankAccountService: BankAccountService) {
	return async function setCard(conversation: SetCardConversation, ctx: Context): Promise<void> {
		...
		while (true) {
			...
			await nextCtx.reply("❌ شماره کارت نامعتبر است. شماره کارت باید دقیقاً ۱۶ رقم باشد. لطفاً مجدداً وارد کنید:", {
				reply_markup: new InlineKeyboard().text("❌ انصراف", "flow:cancel"),
			});
		}
		...
		await ctx.reply("توضیحات تکمیلی (اختیاری) را وارد کنید (یا دکمه [رد شدن] را بزنید):", {
		...
		const additionalNotes = notesCb === "flow:skip" || isSkipCommand(notesText) ? null : notesText.trim();
