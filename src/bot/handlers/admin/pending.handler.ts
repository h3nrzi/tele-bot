...
function buildPendingQueueView(
	...
	const itemsOnPage = pendingRequests.slice(startIndex, startIndex + PENDING_PAGE_SIZE);
...
export async function handlePending(
	...
	const { messageText, keyboard } = buildPendingQueueView(pendingRequests, 1, options?.now);
...
export async function handlePendingPage(
	...
	const { messageText, keyboard } = buildPendingQueueView(pendingRequests, targetPage, options?.now);
 ...
 */
export async function handleReviewCallback(ctx: Context, service: TopUpService): Promise<void> {
	const sender = ctx.from;
	if (!sender) {
		return;
	}
