...
export function createTopUpConversation(
	...
	return async function topup(conversation: TopUpConversation, ctx: Context): Promise<void> {
		...
		while (true) {
			...
			if (callbackData === "flow:cancel" || callbackData === "topup:cancel" || isCancelCommand(text)) {
			...
			await nextCtx.reply(`${validation.message}\n\nلطفاً یک مبلغ معتبر به دلار وارد کنید:`, {
				reply_markup: new InlineKeyboard().text("❌ انصراف", "flow:cancel"),
			});
		}
		...
		try {
			...
			const notesLine = activeAccount.additionalNotes ? `توضیحات: ${activeAccount.additionalNotes}\n` : "";
