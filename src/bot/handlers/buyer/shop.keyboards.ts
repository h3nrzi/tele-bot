...
export function buildShopView(items: CatalogItem[]): ShopView {
	...
	for (const item of items) {
		keyboard.text(`${item.name} - ${formatUsd(item.usdPrice)}`, `shop:item:${item.id}`).row();
	}

	const messageText = `🛍️ فروشگاه خدمات\n\n` + `لطفاً خدمت مورد نظر خود را برای خرید انتخاب کنید:`;
...
export function buildOrderConfirmationView(
	...
	if (hasSufficientBalance) {
		const messageText = baseInvoiceDetails + `آیا از ثبت این سفارش اطمینان دارید؟`;
