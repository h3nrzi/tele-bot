 ...
 */
export async function handleTopUpCommand(ctx: BotContext, deps: TopUpHandlerDependencies): Promise<void> {
	const sender = ctx.from;
	if (!sender) {
		return;
	}

	const { exchangeRateService, exchangeRateConfigService, bankAccountService, buyerService, topUpService, adminIds } =
		deps;
