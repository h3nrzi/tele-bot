...
import { handleMyOrderCommand, handleBuyerCancelOrderCallback } from "@/bot/handlers/buyer/myorder.handler";
...
export function createBuyerComposer(options?: BuyerComposerOptions): Composer<BotContext> {
	...
	const buyerService = options?.buyerService ?? container?.resolve(BuyerService);
	const walletService = options?.walletService ?? container?.resolve(WalletService);
	...
	const exchangeRateConfigService = options?.exchangeRateConfigService ?? container?.resolve(ExchangeRateConfigService);
	...
	composer.hears(
		...
		async (ctx) => {
			...
			await ctx.reply("💰 *مدیریت کیف پول و تراکنش‌ها*\n\nلطفاً یکی از گزینه‌های زیر را انتخاب کنید:", {
				parse_mode: "Markdown",
