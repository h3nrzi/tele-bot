 ...
 * 1. Queries all PENDING top-up requests in ascending creation order.
 * 2. If empty, replies with the empty queue message.
 * 3. If requests exist, presents page 1 of summary list with inline Review buttons and pagination navigation.
 */
export async function handlePending(
  ...
  const pendingRequests = await getPendingRequests(dbClient);
 ...
 * Updates the existing pending queue message to display the requested page.
 */
 ...
 * Re-sends the full receipt details (photo, amounts, inline Approve/Reject buttons)
 * for that specific request to the Admin in a new message.
 */
export async function handleReviewCallback(
  ...
  const req = await getPendingRequestById(requestId, dbClient);
