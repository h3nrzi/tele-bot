...
import type { IOtcPurchaseNotifier } from "./interfaces/otc-purchase.notifier.interface";
import { OtcPurchase } from "./otc-purchase.entity";
import { OtcPurchaseNotFoundError, InvalidOtcPurchaseStateError } from "./otc-purchase.errors";
...
export class OtcPurchaseService {
	...
	public async execute(
		...
		try {
			...
			const orderResult = await this.wallexClient.placeOtcOrder("USDTTMN", "BUY", savedPurchase.usdtQuantity);
	 ...
	 * and re-executes.
	 */
	public async retry(failedPurchaseId: string, dependencies?: OtcPurchaseServiceDependencies): Promise<OtcPurchase> {
		const purchase = await this.otcPurchaseRepo.findById(failedPurchaseId);
		if (!purchase) {
			throw new OtcPurchaseNotFoundError(`OTC purchase with ID ${failedPurchaseId} not found.`);
		}
