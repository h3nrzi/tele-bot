...
export class OtcPurchase {
	...
	constructor(props: OtcPurchaseProps) {
		...
		this._usdtQuantity =
			props.usdtQuantity instanceof UsdAmount ? props.usdtQuantity : new UsdAmount(props.usdtQuantity);
		...
		this._wallexExecutedSum =
			props.wallexExecutedSum !== undefined && props.wallexExecutedSum !== null
				...
				: null;
		this._wallexFee = props.wallexFee !== undefined && props.wallexFee !== null ? BigInt(props.wallexFee) : null;
	...
	public complete(params: CompleteOtcPurchaseParams, now: Date = new Date()): void {
		if (this._status !== "PENDING") {
			throw new InvalidOtcPurchaseStateError(`Cannot complete OTC purchase in status ${this._status}`);
		}
	...
	public fail(errorMessage: string, now: Date = new Date()): void {
		if (this._status !== "PENDING") {
			throw new InvalidOtcPurchaseStateError(`Cannot fail OTC purchase in status ${this._status}`);
		}
		this._status = "FAILED";
