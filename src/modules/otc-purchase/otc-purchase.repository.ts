...
@injectable()
export class DrizzleOtcPurchaseRepository implements IOtcPurchaseRepository<DbExecutor> {
	...
	public async insert(purchase: OtcPurchase, executor?: DbExecutor): Promise<OtcPurchase> {
		...
		try {
			...
			if (err?.code === "23505" || err?.message?.includes("wallex_otc_purchases_top_up_request_id_active_idx")) {
	...
	public async update(purchase: OtcPurchase, executor?: DbExecutor): Promise<OtcPurchase> {
		const db = this.getDb(executor);
	...
	public async findById(id: string, executor?: DbExecutor): Promise<OtcPurchase | null> {
		const db = this.getDb(executor);
		const [row] = await db.select().from(wallexOtcPurchases).where(eq(wallexOtcPurchases.id, id)).limit(1);
	...
	public async findByTopUpRequestId(topUpRequestId: string, executor?: DbExecutor): Promise<OtcPurchase[]> {
		const db = this.getDb(executor);
		const rows = await db
			.select()
	...
	public async findActiveByTopUpRequestId(topUpRequestId: string, executor?: DbExecutor): Promise<OtcPurchase | null> {
		const db = this.getDb(executor);
		const [row] = await db
			.select()
			.from(wallexOtcPurchases)
