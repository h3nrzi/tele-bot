...
export class CatalogService {
	 ...
	 */
	public async createCatalogItem(input: CreateCatalogItemInput, executor?: DbExecutor): Promise<CatalogItem> {
	 ...
	 */
	public async findById(id: string, executor?: DbExecutor): Promise<CatalogItem | null> {
		const client = this.getDb(executor);
		const repo = this.getRepo();
	 ...
	 */
	public async editCatalogItem(id: string, input: EditCatalogItemInput, executor?: DbExecutor): Promise<CatalogItem> {
		const client = this.getDb(executor);
		const repo = this.getRepo();
	 ...
	 */
	public async toggleActive(id: string, executor?: DbExecutor): Promise<CatalogItem> {
		const client = this.getDb(executor);
		const repo = this.getRepo();
	 ...
	 */
	public async deactivate(id: string, executor?: DbExecutor): Promise<CatalogItem> {
	 ...
	 */
	public async reactivate(id: string, executor?: DbExecutor): Promise<CatalogItem> {
		return await this.editCatalogItem(id, { isActive: true }, executor);
	}

	private validateAndFormatPrice(rawPrice: string | number | Decimal | UsdAmount): string {
		let dec: Decimal;
		try {
