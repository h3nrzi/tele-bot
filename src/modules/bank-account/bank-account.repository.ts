...
@injectable()
export class DrizzleBankAccountRepository implements IBankAccountRepository<DbExecutor> {
	...
	public async findActive(executor?: DbExecutor): Promise<BankAccount | null> {
		...
		const [row] = await db.select().from(bankAccounts).where(eq(bankAccounts.isActive, true)).limit(1);
