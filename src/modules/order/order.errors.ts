...
export class InsufficientBalanceForOrderError extends DomainError {
	constructor(message = "Insufficient available balance in buyer wallet to place this order.") {
		super(message, "INSUFFICIENT_BALANCE_FOR_ORDER");
	}
}

export class CatalogItemUnavailableError extends DomainError {
	constructor(message = "The requested catalog item is not available or has been deactivated.") {
		super(message, "CATALOG_ITEM_UNAVAILABLE");
	}
}

export class OrderAlreadyClaimedError extends DomainError {
	constructor(message = "This order has already been claimed by another admin or is no longer in PLACED status.") {
...
export class OrderNotClaimedByAdminError extends DomainError {
	constructor(message = "Only the admin who claimed this order can fulfil it.") {
		super(message, "ORDER_NOT_CLAIMED_BY_ADMIN");
	}
}

export class OrderRejectionNoteRequiredError extends DomainError {
	constructor(message = "A rejection note is mandatory when selecting the OTHER category.") {
