...
@injectable()
export class DrizzleExchangeRateConfigRepository implements IExchangeRateConfigRepository<DbExecutor> {
	...
	public async find(executor?: DbExecutor): Promise<ExchangeRateConfig | null> {
		...
		const [row] = await db.select().from(exchangeRateConfig).limit(1);
	...
	public async upsert(data: UpsertExchangeRateConfigData, executor?: DbExecutor): Promise<ExchangeRateConfig> {
		const db = this.getDb(executor);
		...
		if (existing) {
			...
			if (data.updatedByAdminTelegramId !== undefined)
				updateData.updatedByAdminTelegramId = data.updatedByAdminTelegramId;
