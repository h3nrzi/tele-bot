...
@injectable()
export class DrizzleExchangeRateRepository implements IExchangeRateRepository<DbExecutor> {
	...
	public async findLatest(executor?: DbExecutor): Promise<ExchangeRate | null> {
		...
		const [row] = await db.select().from(exchangeRates).orderBy(desc(exchangeRates.createdAt)).limit(1);
