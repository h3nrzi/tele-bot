 ...
 */
export async function syncBaselineRate(
  deps: SyncBaselineRateDependencies
): Promise<ExchangeRate | null> {
  const logger = deps.logger ?? console;
  try {
    const quote = await deps.wallexClient.getOtcPrice('USDTTMN', 'BUY');
    const rate = await deps.exchangeRateService.setRate(
      deps.botTelegramId,
      quote.priceIrr
    );
  ...
  public async start(
    ...
    if (!interval || interval <= 0) {
      try {
        ...
        const logger = this.logger ?? console;
        logger.error(
          'Failed to read sync interval from config table, falling back to 60 minutes:',
          err
        );
        interval = 60;
      }
