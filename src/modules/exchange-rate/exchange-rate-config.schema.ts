import { pgTable, uuid, numeric, integer, bigint, timestamp, pgEnum, check } from "drizzle-orm/pg-core";
...
export const exchangeRateConfig = pgTable(
	"exchange_rate_config",
	{
		...
		spreadPercent: numeric("spread_percent", { precision: 5, scale: 2 }).default("0.00").notNull(),
		syncIntervalMinutes: integer("sync_interval_minutes").default(60).notNull(),
		updatedByAdminTelegramId: bigint("updated_by_admin_telegram_id", {
			mode: "bigint",
		}),
	...
	(table) => [
		check("exchange_rate_config_spread_check", sql`${table.spreadPercent} >= 0 AND ${table.spreadPercent} <= 10`),
	],
);
