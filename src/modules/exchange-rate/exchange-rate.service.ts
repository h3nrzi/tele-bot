...
export class ExchangeRateService {
	 ...
	 */
	public async setRate(input: SetRateInput, executor?: DbExecutor): Promise<ExchangeRate>;
	...
	public async setRate(
		...
		if (rate <= 0n) {
			throw new InvalidExchangeRateError("Exchange rate (irrPerUsd) must be a positive integer");
		}
	 ...
	 */
	public async getCurrentRate(executor?: DbExecutor): Promise<ExchangeRate | null> {
		const client = executor ?? this.db ?? getDefaultDb();
