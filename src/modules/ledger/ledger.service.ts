...
export class LedgerService {
	...
	public async recordOrderRefund(
		...
		const originalTx = await this.ledgerRepo.findOriginalByOrderId(params.orderId, executor);

		// 2. Create refund transaction
		const refundResult = await this.ledgerRepo.createTransactionWithEntries(
			{
				orderId: params.orderId,
				narrative: params.narrative ?? `Order refund for order ${params.orderId}`,
				entries: [
					{
						accountType: "BUYER_WALLET",
						direction: "CREDIT",
		...
		if (originalTx) {
			await this.ledgerRepo.updateReversedBy(originalTx.id, refundResult.transaction.id, executor);
		}
