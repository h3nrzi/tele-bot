import { pgTable, uuid, numeric, text, timestamp, pgEnum, check, type AnyPgColumn } from "drizzle-orm/pg-core";
import { relations, sql } from "drizzle-orm";
import { topUpRequests } from "@/modules/top-up/top-up.schema";
import { wallets } from "@/modules/wallet/wallet.schema";
import { orders } from "@/modules/order/order.schema";

export const ledgerAccountTypeEnum = pgEnum("ledger_account_type", ["BUYER_WALLET", "SYSTEM_CASH"]);
...
export const ledgerTransactions = pgTable(
	...
	(table) => [
		check("ledger_transactions_source_xor_check", sql`(${table.topUpRequestId} IS NULL) != (${table.orderId} IS NULL)`),
	],
);
