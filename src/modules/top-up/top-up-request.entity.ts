...
export type TopUpStatus = "INITIATED" | "PENDING" | "APPROVED" | "REJECTED" | "EXPIRED" | "CANCELLED";
...
export class TopUpRequest {
	...
	constructor(props: TopUpRequestProps) {
		...
		this._usdAmount = props.usdAmount instanceof UsdAmount ? props.usdAmount : new UsdAmount(props.usdAmount);
		this._irrAmount = props.irrAmount instanceof IrrAmount ? props.irrAmount : new IrrAmount(props.irrAmount);
		this.lockedIrrPerUsd =
			props.lockedIrrPerUsd !== undefined && props.lockedIrrPerUsd !== null
				...
				: this._usdAmount.toDecimal().isZero()
					? 0n
					: BigInt(new Decimal(this._irrAmount.toString()).dividedBy(this._usdAmount.toDecimal()).round().toFixed(0));
	 ...
	 */
	public submitReceipt(fileId: string, caption?: string | null, now: Date = new Date()): void {
	...
	public approve(adminTelegramId: bigint, now: Date = new Date()): void {
		if (this._status !== "PENDING") {
			throw new TopUpRequestNotPendingError("Top-up request is not pending approval or has already been processed.");
		}
	 ...
	 */
	public reject(adminTelegramId: bigint, rejectionReason: string, now: Date = new Date()): void {
		if (this._status !== "PENDING") {
			throw new TopUpRequestNotPendingError("Top-up request is not pending approval or has already been processed.");
		}
	...
	public cancel(now: Date = new Date()): void {
		if (this._status === "PENDING") {
			throw new CannotCancelPendingTopUpError("Cannot cancel top-up request after receipt has been submitted.");
		}
