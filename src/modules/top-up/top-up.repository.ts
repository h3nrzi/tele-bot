...
import { TopUpRequest, type TopUpStatus } from "@/modules/top-up/top-up-request.entity";
import type { RateSource } from "@/modules/top-up/top-up.schema";
import type { ITopUpRequestRepository, PendingTopUpRequestItem } from "@/modules/top-up/top-up.repository.interface";
import { UsdAmount, IrrAmount } from "@/core/shared/money.vo";
import { TOKENS } from "@/core/di/tokens";

@injectable()
export class DrizzleTopUpRequestRepository implements ITopUpRequestRepository<DbExecutor> {
	constructor(@inject(TOKENS.DbClient) private readonly defaultDb?: DbClient) {}
	...
	public async findById(id: string, executor?: DbExecutor): Promise<TopUpRequest | null> {
		const db = this.getDb(executor);
		const [row] = await db.select().from(topUpRequests).where(eq(topUpRequests.id, id));
	...
	public async findByIdForUpdate(id: string, executor: DbExecutor): Promise<TopUpRequest | null> {
		const [row] = await executor.select().from(topUpRequests).where(eq(topUpRequests.id, id)).for("update");
	...
	public async findByIdWithBuyer(id: string, executor?: DbExecutor): Promise<PendingTopUpRequestItem | null> {
		const db = this.getDb(executor);
		const [row] = await db
	...
	public async findPendingWithBuyer(executor?: DbExecutor): Promise<PendingTopUpRequestItem[]> {
		const db = this.getDb(executor);
		const rows = await db
			.select(DrizzleTopUpRequestRepository.PENDING_WITH_BUYER_SELECTION)
			.from(topUpRequests)
	...
	public async findActiveByUserId(userId: string, executor?: DbExecutor): Promise<TopUpRequest | null> {
		const db = this.getDb(executor);
		const [row] = await db
			.select()
			.from(topUpRequests)
			.where(and(eq(topUpRequests.userId, userId), inArray(topUpRequests.status, ["INITIATED", "PENDING"])))
			.limit(1);
	...
	public async findInitiatedByUserId(userId: string, executor?: DbExecutor): Promise<TopUpRequest | null> {
		const db = this.getDb(executor);
		const [row] = await db
			.select()
			.from(topUpRequests)
			.where(and(eq(topUpRequests.userId, userId), eq(topUpRequests.status, "INITIATED")))
			.limit(1);
	...
	public async findLatestByUserId(userId: string, executor?: DbExecutor): Promise<TopUpRequest | null> {
		const db = this.getDb(executor);
		const [row] = await db
			.select()
			.from(topUpRequests)
	...
	public async insert(
		...
		const db = this.getDb(executor);
		const usdStr = data.usdAmount instanceof UsdAmount ? data.usdAmount.toString() : data.usdAmount;
		const irrBigInt = data.irrAmount instanceof IrrAmount ? data.irrAmount.toBigInt() : data.irrAmount;
		...
		const lockedIrr =
			data.lockedIrrPerUsd !== undefined && data.lockedIrrPerUsd !== null
				...
				: usdVo.toDecimal().isZero()
					? 0n
					: BigInt(new Decimal(irrVo.toString()).dividedBy(usdVo.toDecimal()).round().toFixed(0));
