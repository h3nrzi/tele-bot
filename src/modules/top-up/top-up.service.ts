...
export class TopUpService {
	...
	public async initiateTopUp(
		...
		const limits =
			customLimits ?? (process.env.TOPUP_MIN_USD ? TopUpLimits.fromEnv() : this.topUpLimits) ?? TopUpLimits.fromEnv();
		...
		try {
			...
			if (
				...
				throw new ActiveTopUpRequestExistsError("You already have an active top-up request.");
			}
			throw err;
		}
	}
	...
	public async submitReceipt(
		...
		const initiatedRequest = await this.topUpRepo.findInitiatedByUserId(userId, client);

		if (!initiatedRequest) {
			throw new NoInitiatedTopUpRequestError("No active initiated top-up request found.");
		}
		...
		try {
			...
			if (err instanceof TopUpRequestExpiredError) {
				await this.topUpRepo.updateStatus(initiatedRequest.id, "EXPIRED", { updatedAt: now }, client);
			}
			throw err;
		}
	...
	public async approveTopUp(
		...
		const executeApproval = async (tx: DbExecutor): Promise<ApproveTopUpResult> => {
			...
			const updatedWallet = await this.walletRepo.updateBalance(wallet.id, wallet.availableBalance, tx);
		...
		if (dependencies?.notifyBuyer) {
			try {
				...
				console.error(`Failed to send buyer push notification to ${txResult.buyerChatId}:`, notifyErr);
			}
		}
		...
		if (dependencies?.executeOtcPurchase) {
			Promise.resolve()
				.then(() => dependencies.executeOtcPurchase!(txResult.request))
				.catch((otcErr) => {
					console.error(`Failed to execute OTC purchase for top-up request ${txResult.request.id}:`, otcErr);
				});
		}
	...
	public async rejectTopUp(
		...
		if (dependencies?.notifyBuyer) {
			try {
				...
				console.error(`Failed to send buyer rejection push notification to ${txResult.buyerChatId}:`, notifyErr);
			}
		}
	 ...
	 */
	public async getPendingRequests(executor?: DbExecutor): Promise<PendingTopUpRequestItem[]> {
		const client = executor ?? this.db ?? getDefaultDb();
	 ...
	 */
	public async getPendingRequestById(id: string, executor?: DbExecutor): Promise<PendingTopUpRequestItem | null> {
		const client = executor ?? this.db ?? getDefaultDb();
