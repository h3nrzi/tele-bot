...
} from '@/modules/top-up/presentation/admin/pending.messages';
import { getPendingQueueKeyboard } from '@/modules/top-up/presentation/admin/pending.keyboards';
...
export async function handlePending(
  ...
  const pendingRequests = await topUpService.getPendingRequests();
...
export async function handleReviewCallback(
  ...
  const req = await topUpService.getPendingRequestById(requestId);
