...
export class DrizzleWalletRepository implements IWalletRepository<DbExecutor> {
	...
	public async findByUserId(userId: string, executor?: DbExecutor): Promise<Wallet | null> {
	...
	public async findByUserIdForUpdate(userId: string, executor: DbExecutor): Promise<Wallet | null> {
		const [row] = await executor.select().from(wallets).where(eq(wallets.userId, userId)).for("update");
		if (!row) {
			return null;
		}
		return new Wallet({
	...
	public async upsert(
		...
		const db = this.getDb(executor);
		const balanceStr = initialBalance instanceof UsdAmount ? initialBalance.toString() : initialBalance;
	...
	public async updateBalance(walletId: string, newBalance: UsdAmount | string, executor?: DbExecutor): Promise<Wallet> {
		const db = this.getDb(executor);
