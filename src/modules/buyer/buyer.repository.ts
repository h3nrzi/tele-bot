...
import type { IBuyerRepository, UpsertBuyerResult } from "@/modules/buyer/buyer.repository.interface";
...
export class DrizzleBuyerRepository implements IBuyerRepository<DbExecutor> {
	...
	public async findByTelegramChatId(chatId: bigint, executor?: DbExecutor): Promise<Buyer | null> {
		const db = this.getDb(executor);
		const [row] = await db.select().from(users).where(eq(users.telegramChatId, chatId));
		if (!row) {
			return null;
		}
		return new Buyer({
