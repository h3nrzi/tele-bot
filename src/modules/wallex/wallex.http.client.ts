...
import type { WallexSide, WallexOtcQuote, WallexOtcOrderResult } from "./wallex.types";
...
function classifyWallexError(message: string, statusCode?: number, body?: unknown): WallexError {
	const lowerMsg = String(message).toLowerCase();

	if (statusCode === 401 || statusCode === 403 || lowerMsg.includes("unauthorized") || lowerMsg.includes("api key")) {
	...
	if (statusCode === 503 || statusCode === 502 || statusCode === 504 || lowerMsg.includes("maintenance")) {
...
export class WallexHttpClient implements WallexClient {
	...
	public async getOtcPrice(symbol: string, side: WallexSide): Promise<WallexOtcQuote> {
		...
		if (rawPrice === undefined || rawPrice === null || rawPrice === "") {
			throw new WallexBadResponseError(`Wallex price quote missing 'price' field in response: ${JSON.stringify(data)}`);
		}
		...
		try {
			...
			throw new WallexBadResponseError(`Wallex price quote has invalid numeric price '${rawPrice}'`);
		}
	...
	public async placeOtcOrder(
		...
		if (!clientOrderId) {
			throw new WallexBadResponseError(`Wallex order response missing order ID: ${JSON.stringify(data)}`);
		}

		const rawExecutedPrice = resultObj?.executedPrice ?? resultObj?.executed_price ?? resultObj?.price;
		const rawExecutedQty =
			resultObj?.executedQty ?? resultObj?.executed_qty ?? resultObj?.quantity ?? resultObj?.origQty ?? qtyNumber;
		const rawExecutedSum = resultObj?.executedSum ?? resultObj?.executed_sum ?? resultObj?.sum;
		const rawFee = resultObj?.fee ?? resultObj?.commission ?? 0;

		if (rawExecutedPrice === undefined || rawExecutedPrice === null) {
			throw new WallexBadResponseError(`Wallex order response missing executedPrice: ${JSON.stringify(data)}`);
		}
