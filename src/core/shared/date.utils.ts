...
export function formatPersianDateTime(
	...
	const { includeSeconds = true, separator = " ", usePersianDigits = false } = options;
	...
	const timePart = includeSeconds ? `${parts.hour}:${parts.minute}:${parts.second}` : `${parts.hour}:${parts.minute}`;
 ...
 */
export function formatTimeAgo(date: Date | string | number, now: Date = new Date()): string {
	const d = typeof date === "string" || typeof date === "number" ? new Date(date) : date;
